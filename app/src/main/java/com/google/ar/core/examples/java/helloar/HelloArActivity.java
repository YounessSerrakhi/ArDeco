...
public class HelloArActivity extends AppCompatActivity implements SampleRender.Renderer {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    touchHelper = new TouchHelper(/* context= */ this);
  ...
  private void handleTap(Frame frame, Camera camera) {
    ...
    if ((tap != null || swip != null) && camera.getTrackingState() == TrackingState.TRACKING) {
      ...
      if (instantPlacementSettings.isInstantPlacementEnabled()) {
        if(swip!=null) {
          hitResultList = frame.hitTestInstantPlacement(swip.getX(), swip.getY(), APPROXIMATE_DISTANCE_METERS);
          wrappedAnchors.get(wrappedAnchors.size()-1).getAnchor().detach();
          wrappedAnchors.remove(wrappedAnchors.size()-1);
          Log.d("TouchHelper", "handle swip: " + swip.getX() + ", " + swip.getY());
        }else{

        hitResultList = frame.hitTestInstantPlacement(tap.getX(), tap.getY(), APPROXIMATE_DISTANCE_METERS);
        Log.d("TouchHelper", "handleTap: " + tap.getX() + ", " + tap.getY());
        }
      } else {
        if(swip != null) {
          hitResultList = frame.hitTest(swip);
          wrappedAnchors.get(wrappedAnchors.size()-1).getAnchor().detach();
          wrappedAnchors.remove(wrappedAnchors.size()-1);
          Log.d("TouchHelper", "handle swip: " + swip.getX() + ", " + swip.getY());
        }else {
      ...
      for (HitResult hit : hitResultList) {
        ...
        if ((trackable instanceof Plane
          ...
          Log.d("wrappedAnchors", "size: " + wrappedAnchors.size());
